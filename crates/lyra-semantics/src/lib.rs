    ...
    fn check_block_statements(&mut self, statements: &[Statement]) {
        ...
        for statement in statements {
            if unreachable {
                ...
                self.error("unreachable statement", span);
            }
            self.check_statement(statement);
...
mod tests {
    ...
    fn accepts_explicit_unit_return() {
        let analysis =
            analyze_source("fn log() -> Unit { return; } fn main() -> Int { return 0; }");
        assert!(analysis.diagnostics.is_empty());
    }
    ...
    fn accepts_implicit_unit_fallthrough() {
        let analysis =
            analyze_source("fn log() -> Unit { let value = 42; } fn main() -> Int { return 0; }");
        assert!(analysis.diagnostics.is_empty());
    }
    ...
    fn rejects_value_return_from_unit_function() {
        let analysis =
            analyze_source("fn log() -> Unit { return 42; } fn main() -> Int { return 0; }");
        assert!(
            analysis
                .diagnostics
                .iter()
